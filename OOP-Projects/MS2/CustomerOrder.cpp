...
namespace sdds{
   ...
   void CustomerOrder::display(std::ostream& os) const{
        ...
        for (size_t i{0}; i < this->m_cntItem; i++) {
            os << "[" << std::setfill('0') << std::setw(6) << this->m_lstItem[i]->m_serialNumber << "] ";
            os << std::left << std::setfill(' ') << std::setw(m_widthField) << m_lstItem[i]->m_itemName << "   - " << (this->m_lstItem[i]->m_isFilled ? "FILLED" : "TO BE FILLED");
            os << std::endl;
        }
   }
